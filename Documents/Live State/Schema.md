...
**This version supersedes:** `schema.md` v1.11 (04 Aug 2026, earlier today). Pain Input Redesign: `conditionPainScores` values are now a genuine 0-10 continuous input (see field note below) rather than 4 discrete button-representative values. No field shape change — no store.js version change needed for this specifically (already v12 from earlier today's Phase A). `js/data/conditions.js`'s dead, unused `getPainContext()` (a fourth private severity-threshold duplicate, never called anywhere) removed and replaced with the new canonical `getPainBand()`, now the one source of truth for pain-severity display bands app-wide.
