...
**File:** `js/store.js` (confirmed live version: v15, 04 Aug 2026)
**Storage:** `localStorage` key `alongside_user`

**This version supersedes:** `schema.md` v1.13 (04 Aug 2026, earlier today) — which had fallen a step behind: `store.js` had already moved to v14 (new field `pendingDoorRoute`, Phase C follow-up) without a matching Schema.md update at the time. Caught and corrected here, along with this version's own additions: two new fields for Phase D-1, `conditionGoals` and `prescribedExercisesOrigin` — see field notes below. `js/store.js` v14→v15.
