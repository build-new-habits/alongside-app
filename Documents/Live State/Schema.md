...
**File:** `js/store.js` (confirmed live version: v16, 04 Aug 2026)
**Storage:** `localStorage` key `alongside_user`

**This version supersedes:** `schema.md` v1.14 (04 Aug 2026, earlier today). Two additions from the same-day condition-programme build: `prescribedExercises` entries can now carry an optional `conditionId` (documented in that field's existing row, not a new row — the array itself hasn't changed shape at the top level); new field `prescribedExercisesActiveCondition`, a single-use context flag. `js/store.js` v15→v16.
...
| `prescribedExercises` | `array` | `[]` | Entries can carry an optional `conditionId` (**new, 04 Aug 2026**) — scopes a coach-built/coach-recommended/self-built programme to the condition it's for. Additive, nullable; entries added before this existed, or added without a condition context, stay untagged and keep appearing unfiltered in `prescribed.js`. Written by `js/data/conditionProgrammes.js`'s `commitProgramme()` for the two coach routes, or by `prescribed.js` itself (reading `prescribedExercisesActiveCondition`, below) for the manual "Build my own" route. |
