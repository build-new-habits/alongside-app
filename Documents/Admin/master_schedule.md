...
>
> **Live: `alongside-v490`, 134 gates, `store.js` v66, `Schema.md` v1.58, cold start blueprint v92.** Read that blueprint after this block.
>
...
> | **ACK-NAME** | Found while verifying CR-3, not introduced by it. The single-condition onboarding acknowledgement interpolated the raw store id: **"I'll work around wrist-elbow"** — shown to every user declaring exactly one condition, at the moment the coach claims to have listened. Now `getConditionName()`, which already handles the `-acute`/`-subacute` suffixes and falls back to the id rather than rendering `undefined`. `onboarding-thread-data.js` v11. Gated exhaustively across all 31 conditions, not one sample | 🟢 Shipped | w/c 31 Aug |
> | **CR-4a** | **General pre-start statement — built.** Step 14, the only step every person reaches whatever they declared. Says the advice is general and a GP or exercise professional is worth a word first. Phrased as the coach naming its own limit, not as a disclaimer — gated against drifting into legal register, and gated against acquiring urgency language and quietly becoming a triage screen | 🟢 Shipped | w/c 31 Aug |
> | **CR-4b** | **Three-question red-flag screen — deliberately NOT built, and this is a change of plan.** v292 said adopt the statement AND keep the screen. Splitting them: the statement is uncontroversial and shipped; the screen is AI-derived, unverified, and its wording is the exact thing MHRA-SCOPE asks about. Building it before that answer risks building it twice, and shipping unverified triage wording while the regulatory question is open is the worse of the two errors. **Beta ships with the statement and no screen** | 🟠 Held on MHRA-SCOPE | w/c 07 Sep |
> | **CR-5** | Hurt-and-ache guidance. **Widened past the blueprint's rehab-only scope to EVERY exercise** — her word was "always", and someone in a strength session can hurt themselves too. One shared `HURT_AND_ACHE` constant, not 551 copies; renders below the exercise-specific `watchOut` so the non-negotiable safety order is unchanged. Escalation phrasing lifted from SAFEGUARD-1 rather than inventing a third register. `exercise-card.js` v5 | 🟢 Shipped | w/c 31 Aug |
...
>
> **Verification notes from the CLINICAL-RESPONSE session:**
>
> - Nine deliberate breaks proven red across CR-1, CR-2 and ACK-NAME. Gate now at 25 assertions in `tools/verify-clinical-response.mjs`.
> - **Fixture fault, instance ten.** The "scope resolves before Gentle Care" assertion passed on first run with no severe pain score set, so Gentle Care could never have fired and the check proved nothing. The gate now runs a control first and fails loudly if the harness itself is broken. This keeps recurring and the control-first pattern should become standard in new gates.
> - **A gate false-positived on its own correct copy.** The exhaustive ACK-NAME check initially failed on `hypermobility` because CR-3's caveat legitimately uses the word as prose. Narrowed to the interpolated clause. Worth recording: a crude assertion that pushes someone to weaken real copy to satisfy a gate is worse than no gate.
> - Two sessions ran concurrently on `main`. Rebase, full suite re-run, then push — not push then check.
>
...
>
> ⚫ **DEV_PANEL_ENABLED deliberately NOT flipped.** *(Re-confirmed 06 Sep from the repo after a session again raised it from stale notes. It is closed.)* An earlier claim in this session — that the switcher "ships to every user who can find it" — **was wrong and is withdrawn.** The panel renders `hidden aria-hidden="true"` and needs a deliberate triple-tap that is documented nowhere in user-facing copy. The A1 decision of 13 Aug stands: it stays through beta because testers need it, and flips false before **public launch, January 2027**. Flipping it now would freeze every device in whatever tier it is already wrongly in and fix nothing.
>
