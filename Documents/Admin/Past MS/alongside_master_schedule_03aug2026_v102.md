...
- [ ] 🟠 **NEW, recommended priority — Tier-gating build.** Not yet scoped as its own session, but flagged as highest priority of the remaining things the Supabase design session unblocks (see above): `js/auth.js` doesn't exist, and `session-builder-ui.js` has a live bug (found 03 Aug) locking paying Personal/Athlete users out of features they've paid for. (`upgrade.js`'s crash, previously listed here, was fixed 03 Aug — see dashboard.) A real enforcement layer (S4-TG, 9 May 2026) was fully scoped but never built.
