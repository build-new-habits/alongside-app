...
**This version's substantive changes (03 Aug, extended session):** Website session, run in full directly in the PM chat across multiple rounds — WCAG audit, `/upgrade` page build, live-screenshot follow-ups, an accessibility audit, a full comparison-table rework, and finally a ground-truth correction pass against live `alongside-app` code. Full detail in the section below. Headline items beyond the original WCAG audit: (1) `/upgrade` and new `/who-its-for/` pages built, gold token lightened, footer "Ltd" fixed; (2) real accessibility gap found and fixed — comparison-table row labels had no programmatic link to their data cells; (3) **the comparison table was checked against live code and found to be describing spec'd-but-unbuilt features as if live** — no difficulty-based exercise gating exists at all, no hardcoded programme exists any more, no generative/periodised programme engine exists — all corrected; (4) new build decision from Graeme: migrate the exercise difficulty scale from the live 1–3 to 1–5, with users able to start at any level and progress — needs its own blueprint.
