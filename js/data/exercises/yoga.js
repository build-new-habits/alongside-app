 ...
 *   were the only untagged entries in the database, and every read in
 *   the codebase was `(ex.difficultyLevel || 1)` -- treating them as the
 *   EASIEST possible, which is backwards for safety. A 76-year-old with
 *   low confidence was traced being served Warrior III, a single-leg
 *   balance pose and a genuine fall risk for her.
 *
 *   Tagged on SKILL AND BALANCE DEMAND, not on energy cost: a
 *   restorative pose is low energy and low skill, while Warrior III is
 *   moderate energy and high balance demand. Different axes.
 *
 * 11 Aug 2026 v3
 *
