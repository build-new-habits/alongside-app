 ...
 * was served "Resisted Sprint — Partner or Sled" inside a gym Lower Body
 * session. It arrives legitimately: category 'cardio', movementPattern
 * 'locomotion', affectsAreas glutes and hamstring, so a lower-body
 * matcher takes it. Nothing distinguished sled sprints and agility
 * ladders from ordinary conditioning, because nothing needed to until
 * gym sessions started drawing from the same pool.
 *
 * Same finding shape as C2 (rehabilitation), C3 (seated) and FIX-1
 * (yoga/pilates): the content is good, it is correctly tagged for what
 * it IS, and nothing ever asked whether it belongs in the session being
 * built.
 *
 * De-prioritised in gym-shaped sessions only, and as a preference --
 * personas 2.3 (national-standard sprinter) and 2.9 (academy
 * footballer) exist and this is exactly their content. It remains fully
