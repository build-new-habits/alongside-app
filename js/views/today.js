 ...
 *
 * 30 May 2026 v2 --- Toggle bug fixes:
 *   Master weekly plan toggle: stopPropagation() added to prevent label
 *   click bubbling to parent listeners. Direct input change handler.
 *   Per-day toggles: same fix. Event attached after each rerender.
 *
      ...
      ` : `
        ...
        <div class="card" style="margin-bottom:var(--space-4);">
          <div style="display:flex;justify-content:space-between;align-items:center;">
            ...
            <label class="toggle-switch" id="weekly-plan-toggle-label"
                   aria-label="Enable weekly plan">
...
function wirePanel() {
  ...
  // Use the input directly; stopPropagation on label prevents double-fire
  const masterToggleLabel = document.getElementById("weekly-plan-toggle-label");
  if (masterToggleLabel) {
    masterToggleLabel.addEventListener("click", e => e.stopPropagation());
  }
  const masterToggle = document.getElementById("weekly-plan-toggle");
  if (masterToggle) {
    masterToggle.addEventListener("change", () => {
      store.set("weeklyPlanEnabled", masterToggle.checked);
      rerenderMyWeek();
    });
  }
  ...
  // Stop label click propagation so it doesn't trigger row open/close
  document.querySelectorAll(".weekly-plan-row-toggle .toggle-switch").forEach(label => {
