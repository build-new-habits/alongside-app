 ...
 *
 * v1.6 — Prescribed exercises section reconnected (S3-6):
 *   renderPrescribedSection() added below workout options.
 *   Empty state: soft CTA card with inline add-exercise form.
 *   Populated state: summary card (count, credits available) + Start Session button.
 *   Add form: name, sets, reps/hold time, notes — free text, no DB lookup.
 *   Credits: 35 per exercise, max 150 (matching prescribed-session.js constants).
 *   Checkin shortcut flag: if store key "openPrescribedForm" is true on mount,
 *   the add form opens automatically (ADHD-aware design from S3-2).
 *
 ...
 *
 * Two states:
 *   Empty  — soft CTA card; user can open the add-exercise form.
 *   Filled — summary card showing active count + credits available,
 *            with a Start Session button (navigates to prescribed-session).
 *
 * Credits: 35 per exercise, capped at 150 for the session.
 * "Active" means: not completedToday.
 *
 * The add-form (id="prescribed-add-form") is rendered hidden inside the
 * empty-state card and toggled by renderPrescribedSection's CTA button.
 * This avoids a router round-trip just to show a form.
 */
function renderPrescribedSection() {
  const all    = store.get("prescribedExercises") || [];
  const active = all.filter(e => !e.completedToday);
  const done   = all.filter(e => e.completedToday);

  const CREDITS_PER = 35;
  const CREDITS_MAX = 150;
  const creditsAvail = Math.min(active.length * CREDITS_PER, CREDITS_MAX);

  const addFormHtml = `
    <form id="prescribed-add-form" class="prescribed-add-form" aria-label="Add prescribed exercise" novalidate>
      <div class="form-field">
        <label class="form-label" for="px-name">Exercise name <span aria-hidden="true">*</span></label>
        <input
          type="text"
          id="px-name"
          name="px-name"
      ...
      <div class="form-field">
        <label class="form-label" for="px-notes">Notes from your physio (optional)</label>
        <textarea
          id="px-notes"
          name="px-notes"
      ...
      <div class="prescribed-form-actions">
        <button type="submit" class="btn btn-primary btn-full" id="px-save-btn">
          Add Exercise
        </button>
        <button type="button" class="btn btn-ghost btn-full" id="px-cancel-btn">
          Cancel
        </button>
      </div>
    </form>
  ...
  if (active.length === 0) {
    return `
      <div class="card prescribed-section" id="prescribed-section" style="margin-top: var(--space-6);">
        <div class="prescribed-header">
          <span class="prescribed-icon" aria-hidden="true">🩺</span>
          <div>
            <h3>Prescribed Exercises</h3>
            <p class="text-sm text-muted">Exercises from your physio or consultant</p>
          </div>
        </div>
        ...
        <div id="prescribed-empty-cta">
          <button class="btn btn-ghost btn-full" id="px-open-form-btn"
                  aria-expanded="false" aria-controls="prescribed-add-form">
            + Add prescribed exercise
          </button>
        </div>

        <div id="prescribed-form-wrap" class="hidden" aria-live="polite">
          ${addFormHtml}
        </div>
      </div>
    `;
  ...
  return `
    <div class="card prescribed-section" id="prescribed-section" style="margin-top: var(--space-6);">
      <div class="prescribed-header">
        ...
        <div>
          <h3>Prescribed Exercises</h3>
          <p class="text-sm text-muted">${active.length} to do today &mdash; up to +${creditsAvail} &#11088;</p>
        </div>
      </div>

      <ul class="prescribed-list" aria-label="Prescribed exercises">
        ${active.map(ex => `
          <li class="prescribed-list-item">
            <span class="prescribed-list-name">${ex.name}</span>
            <span class="prescribed-list-detail text-sm text-muted">
              ${ex.sets ? ex.sets + " sets" : ""}${ex.sets && ex.reps ? " &times; " : ""}${ex.reps || ""}
            </span>
          </li>
        `).join("")}
      </ul>
      ...
      <button class="btn btn-primary btn-full" id="px-start-session-btn"
              style="margin-top: var(--space-4);"
              aria-label="Start prescribed exercise session">
        Start Prescribed Session
      </button>

      <div id="prescribed-form-wrap" class="hidden" style="margin-top: var(--space-4);" aria-live="polite">
        ${addFormHtml}
        <button class="btn btn-ghost btn-full" id="px-open-form-btn"
                aria-expanded="false" aria-controls="prescribed-add-form"
      ...
      <div id="prescribed-empty-cta" style="margin-top: var(--space-3);">
        <button class="btn btn-ghost btn-full" id="px-open-form-btn"
                aria-expanded="false" aria-controls="prescribed-add-form">
          + Add another exercise
        </button>
...
export function onMount() {
  ...
  // ── Prescribed section ────────────────────────────────────────────────────
  mountPrescribedSection();
}

// ── Prescribed section mount ───────────────────────────────────────────────────

/**
 * Wire up all prescribed-section interactivity.
 *
 * Responsibilities:
 *   - Open/close the add-exercise form
 *   - Handle form submission (write to store, re-render)
 *   - Handle Start Session button
 *   - Auto-open form if store flag "openPrescribedForm" is true
 *     (set by checkin.js shortcut button — ADHD-aware design)
 */
function mountPrescribedSection() {
  // ── Auto-open from checkin shortcut ───────────────────────────────────────
  // checkin.js sets this flag when the user taps the prescribed shortcut.
  // We consume it once then clear it so it does not re-trigger on next visit.
  ...
  // ── Open form button ──────────────────────────────────────────────────────
  // There may be multiple "open form" buttons (empty state + populated state).
  // Use event delegation on the section container.
  document.getElementById("prescribed-section")?.addEventListener("click", e => {
    if (e.target.closest("#px-open-form-btn") || e.target.id === "px-open-form-btn") {
      openPrescribedForm();
    }
  });

  // ── Cancel form ───────────────────────────────────────────────────────────
  document.getElementById("px-cancel-btn")?.addEventListener("click", closePrescribedForm);

  // ── Form submission ───────────────────────────────────────────────────────
  document.getElementById("prescribed-add-form")?.addEventListener("submit", e => {
    e.preventDefault();
    savePrescribedExercise();
  });
}

function openPrescribedForm() {
  const wrap = document.getElementById("prescribed-form-wrap");
  const cta  = document.getElementById("prescribed-empty-cta");
  const btn  = document.getElementById("px-open-form-btn");
  ...
  // Move focus to the name field for accessibility
  setTimeout(() => {
...
function savePrescribedExercise() {
  ...
  // Build a unique ID — timestamp-based, safe for localStorage
  const id = "px-" + Date.now();

  const exercise = {
    id,
    exerciseId:    null,       // no DB lookup — free entry
    name,
    description:   "",
    frequency:     "",
    prescribedBy:  "",
  ...
  // Re-render the Today view to show the updated list
  router.navigate("today");
}
