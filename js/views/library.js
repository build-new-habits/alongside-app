...
const GUIDED_CATEGORIES = [
  ...
  },
  {
    ...
    sessions: [
      ...
      { label: "Core",          icon: "\uD83E\uDDD8",  target: "core-session",   note: "" },
      { label: "Upper body",    icon: "\uD83D\uDCAA",  target: "gym-programme",  note: "" },
      { label: "Lower body",    icon: "\uD83E\uDDB5",  target: "gym-programme",  note: "" },
      { label: "Strength",      icon: "\uD83D\uDD25",  target: "gym-programme",  note: "" },
...
function navigateToSession(target, quiet) {
...
export function onMount() {
  ...
  document.querySelectorAll("[data-target]").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      if (!target) return;
      navigateToSession(target, quiet);
    });
  });
