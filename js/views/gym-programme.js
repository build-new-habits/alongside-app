 ...
 *
 * Fixes (20 May 2026):
 *   - mountSessionGuard() added to onMount()  intercepts device back gesture
 *     during active session. Shows coach-voiced confirmation card.
 *   - dismountSessionGuard() added to resetSession()  cleans up listener.
 *   - savePartialGymSession() removed  partial save now handled by the
 *     onExit lambda inside mountSessionGuard().
 *   - _sessionStartTime added for accurate partial session duration logging.
 *
...
function wireEvents() {
  ...
  // Back button -- shown in header, navigates to settings
...
//  Exit confirmation (tapped from Back button during active session) 
...
function showExitConfirm() {
  ...
  document.getElementById("exit-confirm-leave")?.addEventListener("click", () => {
    ...
    const elapsed = Math.round((Date.now() - _sessionStartTime) / 60000);
    log.push({
      type:         "gym-programme",
      status:       "partial",
