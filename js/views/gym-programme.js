...
import { mountSessionGuard, dismountSessionGuard, showExitCard, rearmGuard } from "../session-guard.js";
        ...
        <div class="gym-card-panel" id="panel-${safeId}">
          ...
          ${!isDone ? `
            <div class="gym-skip-row">
              <button class="btn btn-ghost btn-small gym-skip-btn"
                      data-exercise-id="${ex.id}"
                      data-session="${sessionId}"
                      data-exercise="${ex.name}"
...
function rerender() {
  ...
  if (main) {
    ...
    wireEvents();
    rearmGuard(); // re-arm back gesture intercept after DOM rebuild
  }
}
...
function wireEvents() {
  ...
  view.addEventListener("click", e => {
    ...
    if (skipBtn) {
      ...
      const sid    = skipBtn.dataset.session;
      const exName = skipBtn.dataset.exercise;
      store.set("gymSkip_" + sid + "_" + exName.replace(/[^a-zA-Z0-9]/g, "_"), "skipped");
      const session = PROGRAMME.sessions.find(s => s.id === sid);
      const nextEx  = session?.exercises.find(e => !completedIds.has(e.id) && e.id !== eid);
      expandedId    = nextEx ? nextEx.id : null;
      rerender();
      return;
    }
